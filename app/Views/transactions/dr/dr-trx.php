...
$claim_tag ='';
$mtkn_vend = '';
$claims_revised_date = date('Y-m-d');
if(!empty($mtkn_trxno)) { 
    $str = "SELECT
        aa.`trx_no`,
        aa.`supplier_id`,
    ...
    $is_validated = $rw['is_validated'];
    $is_verified  = $rw['is_verified'];
    $is_reviewed =$rw['is_reviewed'];
    $mtknImg_isver        = './uploads/rcv_claims/'.$rw['verified_rcpt'];
    $mtknImg_isreview = './uploads/rcv_claims/'.$rw['reviewed_rcpt'];
    $qq->freeResult();
}
			...
			<div class="col-md-6">
				...
				<div class="row mb-3 form-group">
					<div class="col-sm-4">
						...
						<?php if(!empty($file )):?>
						<div class="">
							<div class="text-center border p-2 rounded bg-dlight" style="background-color:#ababab78;"> 
								<h6>Attached File</h6>
							<div class="text-center">
							<span class="hover_dito fa fa-file-image-o " style="font-size: 100px;"></span>
								</div>
								<div class="text-center">
								...
								<!--   <button  style="font-size: 10px;" type="button" class="btn btn-sm btn-info vw_files_" title = "View files"value="<?=$mtknImg;?>"><i class="fa fa-file"></i> FILES</button> -->
								<a  style="font-size: 10px;"  onclick ="window.open('<?=site_url().$mtknImg;?>')" class="btn btn-info btn-sm" title = "View"><i class="fa fa-file text-white"></i> View </a>
								<form method='post' action='<?= base_url() ?>MyRFP/myfilesdl_1'>
									<input type="hidden" name="data_01" value="<?=$mtknImg?>">
								<!--       <button   style="font-size: 10px;" type="submit" class="btn btn-sm btn-warning dl_files_" title = "Download files" value="<?=$mtknImg;?>"><i class="fa fa-download"></i> Download</button> -->
								</form>

								<?php else: ?>
								<label class="btn btn-danger btn-sm" style="font-size: 10px;"  >
									<span class="fa fa-upload"> </span> Browse <input data-id="__lbl01" accept="image/gif,image/jpeg,image/png,application/pdf" class="__pld_file_img01" size="5" id="__pld_file_img01" type="file" multiple name="images[]" style="display: none;" >
								</label>
									<?php endif;?>
								</div>
							</div>
					...
					<div class="col-sm-4">
						<!-- verified file upload row ml-2 -->

						<?php if(!empty($file) && ($claim_tag == 'Y' && $is_validated == 'Y' && $cuserrema ==='H' && $claims_revised_date >= '2023-11-01')):?>
						<div class="">
							<div class="text-center border p-2 rounded bg-dlight" style="background-color:#ababab78;"> 
								<h6>For CAD: Attached File</h6>
							<div class="text-center">
							<span class="hover_dito fa fa-file-image-o " style="font-size: 100px;"></span>
								</div>
								<div class="text-center">
									<label style ="font-size:15px;"id="__lbl01_isver"></label>
								</div>
								<div class="d-flex justify-content-center">
								<?php if( $is_verified == "Y"):?>
								
								<!--   <button  style="font-size: 10px;" type="button" class="btn btn-sm btn-info vw_files_" title = "View files"value="<?=$mtknImg;?>"><i class="fa fa-file"></i> FILES</button> -->
								<a  style="font-size: 10px;"  onclick ="window.open('<?=site_url().$mtknImg_isver;?>')" class="btn btn-info btn-sm" title = "View"><i class="fa fa-file text-white"></i> View </a>
								<form method='post'>
									<input type="hidden" name="data_01_isver" value="<?=$mtknImg_isver?>">
								<!--       <button   style="font-size: 10px;" type="submit" class="btn btn-sm btn-warning dl_files_" title = "Download files" value="<?=$mtknImg;?>"><i class="fa fa-download"></i> Download</button> -->
								</form>

								<?php else: ?>
								<label class="btn btn-danger btn-sm" style="font-size: 10px;"  >
									<span class="fa fa-upload"> </span> Browse <input data-id="__lbl01_isver" accept="image/gif,image/jpeg,image/png,application/pdf" class="__pld_file_img01_isver" size="5" id="__pld_file_img01_isver" type="file" multiple name="images_isver[]" style="display: none;" >
								</label>
								<button style="font-size: 10px;" class="btn btn-info btn-sm" id="mbtn_mn_Verify" <?php echo $dis3; ?> type="submit">Verify Claims</button>
								<?php endif;?>
								</div>
						...
						<div class="col-sm-4">
						<!-- review file upload -->
						<?php if(!empty($file) && ($claim_tag == 'Y' && $is_validated == 'Y' && $is_verified == 'Y' && $cuserrema ==='H' && $claims_revised_date >= '2022-11-01')):?>
						<div class="">
							<div class="text-center border p-2 rounded bg-dlight" style="background-color:#ababab78;"> 
								<h6>For SOD: Attached File</h6>
							<div class="text-center">
							<span class="hover_dito fa fa-file-image-o " style="font-size: 100px;"></span>
								</div>
								<div class="text-center">
									<label style ="font-size:15px;"id="__lbl01_isreview"></label>
								</div>
								<div class="d-flex justify-content-center">
								<?php if( $is_reviewed == "Y"):?>
								
								<!--   <button  style="font-size: 10px;" type="button" class="btn btn-sm btn-info vw_files_" title = "View files"value="<?=$mtknImg;?>"><i class="fa fa-file"></i> FILES</button> -->
								<a  style="font-size: 10px;"  onclick ="window.open('<?=site_url().$mtknImg_isreview;?>')" class="btn btn-info btn-sm" title = "View"><i class="fa fa-file text-white"></i> View </a>
								<form method='post'>
									<input type="hidden" name="data_01_isreview" value="<?=$mtknImg_isreview?>">
								<!--       <button   style="font-size: 10px;" type="submit" class="btn btn-sm btn-warning dl_files_" title = "Download files" value="<?=$mtknImg;?>"><i class="fa fa-download"></i> Download</button> -->
								</form>

								<?php else: ?>
								<label class="btn btn-danger btn-sm" style="font-size: 10px;"  >
									<span class="fa fa-upload"> </span> Browse <input data-id="__lbl01_isreview" accept="image/gif,image/jpeg,image/png,application/pdf" class="__pld_file_img01_isreview" size="5" id="__pld_file_img01_isreview" type="file" multiple name="images_isreview[]" style="display: none;" >
								</label>
								<button style="font-size: 10px;" class="btn btn-info btn-sm" id="mbtn_mn_Review" <?php echo $dis3; ?> type="submit">Review Claims</button>
								<?php endif;?>
								</div>
...
<script type="text/javascript"> 
   ...
   function meconfirmdel(smuid){ 
	...
	$('#mbtn_mn_Verify').on('click',function(){
        try{
            __mysys_apps.mepreloader('mepreloaderme',true);
            my_data = new FormData();
            ...
            if(trxno_id != ''){
                ...
                if (filesCount_isver == 0 ){
                    alert('For CAD: Please select file to upload.');
                    __mysys_apps.mepreloader('mepreloaderme',false);
                    return false;
                }

               if(filesCount_isver >1 ){
                        alert('For CAD: Kindly compile the image into pdf format');
                        __mysys_apps.mepreloader('mepreloaderme',false);
                        return false;
            ...
            if(trxno_id != ''  ){
                ...
                jQuery.ajax({ 
                    type: "POST",
                    url: '<?=site_url()?>dr-trx-rcv-claims-verify',
                    context: document.body,
                    data: my_data,
                    contentType: false,
                    global: false,
		...
		$('#mbtn_mn_Review').on('click',function(){
        try{
			__mysys_apps.mepreloader('mepreloaderme',true);
            my_data = new FormData();
            ...
            var filerfp_isreview      = '__pld_file_img01_isreview'; //CAD  attach
            my_data.append('fld_txttrx_no',fld_txttrx_no);
            my_data.append('_hdrid_mtkn',trxno_id);
            var filerfps_isreview    = $('.'+filerfp_isreview);
            var filesCount_isreview   = 0;
            ...
            if(trxno_id != ''){
                if(userrema =='H'){
                $.each(filerfps_isreview, function(i,filerfp_isreview){
                    if(filerfp_isreview.files.length > 0 ){
                        $.each(filerfp_isreview.files, function(k,file_isreview){
                            my_data.append('images_isreview[]', file_isreview);
                            filesCount_isreview++;
                ...
                if (filesCount_isreview == 0 ){
                    alert('For SOD: Please select file to upload.');
                    __mysys_apps.mepreloader('mepreloaderme',false);
                    return false;
                }

               if(filesCount_isreview >1 ){
                        alert('For SOD: Kindly compile the image into pdf format');
                        __mysys_apps.mepreloader('mepreloaderme',false);
                        return false;
            ...
            if(trxno_id != ''  ){
                ...
                jQuery.ajax({ 
                    type: "POST",
                    url: '<?=site_url()?>dr-trx-rcv-claims-review',
                    context: document.body,
                    data: my_data,
                    contentType: false,
                    global: false,
