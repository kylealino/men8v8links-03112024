...
<div class="card">
	<div class="card-header mb-3">
		<h3 class="h4 mb-0"> <i class="bi bi-list-ul"></i> Cancelled Dr Records </h3>
	</div>
	...
	<div class="card-body">
		<div class="pt-2  mt-2"> 
			<nav class="nav nav-pills flex-column flex-sm-row border  gap-1 fw-bold">
				...
				<a id="anchor-cwo" class="flex-sm-fill text-sm-center  active p-2 bg-light rounded-left" > <i class="bi bi-truck"> </i> Cancelled Crossdocking </a>
				<a id="anchor-mn" class=" flex-sm-fill text-sm-center   p-2  bg-light "><i class="bi bi-journal-text"></i> Cancelled Manual SD </a>
				<a id="anchor-tap" class=" flex-sm-fill text-sm-center   p-2 bg-light rounded-right"><i class="bi bi-truck-front"></i> Cancelled TAP </a>
                <a id="anchor-crpl" class=" flex-sm-fill text-sm-center   p-2 bg-light rounded-right"><i class="bi bi-truck-front"></i> Cancelled OLD SD </a>
			</nav>
		</div>

		<div id="cancelled-vw" class="text-center p-2 rounded-3  mt-3 border-dotted bg-light p-4 ">
			<?php
			?> 
		</div> 
	</div> 
...
<script type="text/javascript"> 
     ...
     $(document).ready(function() {
        ...
        cancelleddr_view_recs(sd_type,fld_d2brnch,fld_d2dtfrm,fld_d2dtto,fld_brancharea);
    });
    ...
    function cancelleddr_view_recs(sd_type,fld_d2brnch,fld_d2dtfrm,fld_d2dtto,fld_brancharea){ 
        var ajaxRequest;

        ajaxRequest = jQuery.ajax({
            url: "<?=site_url();?>mydashb-dr-cancelled-recs",
            type: "post",
            data: {
                sd_type: sd_type,
        ...
        ajaxRequest.done(function(response, textStatus, jqXHR) {
            jQuery('#cancelled-vw').html(response);
