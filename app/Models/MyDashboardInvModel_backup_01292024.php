...
class MyDashboardInvModel extends Model
{
    ...
    public function __construct() { 
        ...
        $this->db_erp2 = $this->myusermod->mydbname->medb(2);
    ...
    public function dashb_inv_intransit_rcv(){
        ...
        $str="
            CREATE TABLE IF NOT EXISTS pansamantala.`trx_manrecs_hd_tmp` LIKE ap2.`trx_manrecs_hd`;
        ";
        $q = $this->mylibzdb->myoa_sql_exec2($str,'URI: ' . $_SERVER['PHP_SELF'] . chr(13) . chr(10) . 'File: ' . __FILE__  . chr(13) . chr(10) . 'Line Number: ' . __LINE__);
        
        $str = "
        insert into pansamantala.`trx_manrecs_hd_tmp`
            (`trx_no`,
            `comp_id`,
            `branch_id`,
            `drno`,
        ...
        $str="
        CREATE TABLE IF NOT EXISTS pansamantala.`trx_manrecs_dt_tmp` LIKE ap2.`trx_manrecs_dt`;
        ";
        $q = $this->mylibzdb->myoa_sql_exec2($str,'URI: ' . $_SERVER['PHP_SELF'] . chr(13) . chr(10) . 'File: ' . __FILE__  . chr(13) . chr(10) . 'Line Number: ' . __LINE__);
        
        //insert details
        $str_qry = "
        INSERT INTO pansamantala.`trx_manrecs_dt_tmp` (
            `mrhd_rid`,
            `trx_no`,
            `mat_rid`,
            `mat_code`,
