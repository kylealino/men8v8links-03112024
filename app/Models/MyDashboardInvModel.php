...
class MyDashboardInvModel extends Model
{
    ...
    public function dashb_inv_recs($npages = 1,$npagelimit = 10,$msearchrec=''){
        ...
        if(!empty($report)):
            switch ($report) {
                ...
                break;
                case '12':
                    $optn_rpt = "AND aa.`hd_sm_tags` = 'D' AND ((aa.`counter_tag` = 'N' AND aa.`claim_tag` = 'N')  OR ((aa.`claim_tag` = 'Y') AND (aa.`is_validated` = 'Y') AND (IF(date(aa.`encd_date`) >= DATE('$claims_revised_date'),aa.`is_verified` = 'Y',aa.`is_verified` = 'N')) AND (IF(date(aa.`verified_date`) >= DATE('$claims_revised_date2'),aa.`is_reviewed` = 'Y',aa.`is_reviewed` = 'N'))))"; //FOR COUNTERED
                break;
                case '13':
    ...
    public function dashb_inv_process(){
        ...
        if((!empty($fld_d2dtfrm) && !empty($fld_d2dtto)) ){
            ...
            $str_date_crpl = " AND (date(`date_encd`) >= date('{$fld_d2dtfrm}') AND  date(`date_encd`) <= date('{$fld_d2dtto}'))";
        ...
        $str ="
            ...
            $str="
                ...
                (
                    ...
                    (SELECT COUNT(aa.`recid`) intransit_mn FROM `trx_manrecs_mo_hd` aa JOIN `mst_companyBranch` cc ON (aa.`branch_id` = cc.`recid`) WHERE aa.`is_receive` = 'N' AND is_cancelled = 'N' {$str_date_mo} {$optn_branch_mo} {$str_branchgrparea}) ll,
                    (SELECT COUNT(aa.`recid`) intransit_cwo FROM `warehouse_shipdoc_hd` aa JOIN `mst_companyBranch` cc ON (aa.`brnch_rid` = cc.`recid`) WHERE aa.`is_intransit` = 'Y' AND aa.`is_receive` = 'N' AND is_cancelled = 'N' and aa.`done` = '1' {$str_date_cwo} {$optn_branch_cwo} {$str_branchgrparea}) mm,
                    (SELECT COUNT(aa.`recid`) intransit_crpl FROM `trx_crpl` aa JOIN `mst_companyBranch` cc ON (aa.`brnch_rid` = cc.`recid`) WHERE aa.`is_receive` = 'N' AND is_cancelled = 'N' {$str_date_crpl} {$optn_branch_crpl} {$str_branchgrparea}) nn,
                    ...
                    (SELECT COUNT(aa.`is_cancelled`) cancelled_crpl FROM `trx_crpl` aa JOIN `mst_companyBranch` cc ON (aa.`brnch_rid` = cc.`recid`) WHERE aa.`is_cancelled` = 'Y' {$str_date_crpl} {$optn_branch_crpl} {$str_branchgrparea}) tt
                )
            ";
            $qry = $this->mylibzdb->myoa_sql_exec2($str,'URI: ' . $_SERVER['PHP_SELF'] . chr(13) . chr(10) . 'File: ' . __FILE__  . chr(13) . chr(10) . 'Line Number: ' . __LINE__);
            $row = $qry->getRowArray();
            ...
            $cancelled_crpl  	  = $row['cancelled_crpl'];
            ...
            $str="
                ...
                (
                    ...
                    (SELECT COUNT(aa.`recid`) intransit_tap FROM `tap_shipdoc_hd` aa JOIN `mst_companyBranch` cc ON (aa.`brnch_rid` = cc.`recid`) WHERE aa.`is_intransit` = 'Y' AND aa.`is_receive` = 'N' AND is_cancelled = 'N' AND aa.`done` = '1' {$str_date_tap} {$optn_branch_tap} {$str_branchgrparea}) qq,
            ...
            $total_cancelled = ($cancelled_cwo + $cancelled_mn + $cancelled_tap + $cancelled_crpl);
    ...
    public function dashb_inv_created_recs($npages = 1,$npagelimit = 10,$msearchrec=''){
        ...
        if ($sd_type == 'CWO') {
            ...
            FROM 
                trx_crpl
            WHERE
                DATE(`date_encd`) >= '2024-01-01'
                {$str_optn} {$optn_branch_crpl}
            ";
        }
    ...
    public function dashb_inv_intransit_recs($npages = 1,$npagelimit = 10,$msearchrec=''){
        ...
        if ($sd_type == 'CWO') {
            ...
            FROM 
                warehouse_shipdoc_hd
            WHERE 
                `is_intransit` = 'Y' AND `is_receive` = 'N' AND is_cancelled = 'N'
            AND
                DATE(`done_date`) >= '2024-01-01'
            ...
            FROM 
                trx_manrecs_mo_hd
            WHERE
                `is_receive` = 'N' AND is_cancelled = 'N' AND
                DATE(`mo_date`) >= '2024-01-01'
            {$str_optn} {$optn_branch_mo}
            ";
        }elseif ($sd_type == 'TAP') {
            ...
            FROM 
                tap_shipdoc_hd
            WHERE
                `is_intransit` = 'Y' AND `is_receive` = 'N' AND is_cancelled = 'N' AND
                DATE(`done_date`) >= '2024-01-01'
            {$str_optn} {$optn_branch_tap}
            ";
        }else{
    ...
    public function dashb_inv_intransit_rcv(){
        ...
        //SET SUPPLIER TO GOLDENWIN EMPIRE TSHIRT
        // if ($sd_type == 'TA') {
        //     $str="
        //     SELECT `recid` FROM `mst_vendor` WHERE `recid` = '4773'
        //     ";
        //     $q = $this->mylibzdb->myoa_sql_exec($str,'URI: ' . $_SERVER['PHP_SELF'] . chr(13) . chr(10) . 'File: ' . __FILE__  . chr(13) . chr(10) . 'Line Number: ' . __LINE__);
        //     $rw = $q->getRowArray();
        //     $fld_supplier = $rw['recid'];
        // }else{//SET SUPPLIERT TO FINE DYNASTY
        //     $str="
        ...
        if ($sd_type == 'TA') {
            $str="
                SELECT `supplier_id` FROM tap_shipdoc_hd WHERE crpl_code = '$trxno'
            ";
            $q = $this->mylibzdb->myoa_sql_exec($str,'URI: ' . $_SERVER['PHP_SELF'] . chr(13) . chr(10) . 'File: ' . __FILE__  . chr(13) . chr(10) . 'Line Number: ' . __LINE__);
            //IF TAP SHIPDOC SUPPLIER IS EMPTY, SET ID = 0
            if($q->resultID->num_rows == 0):
                $fld_supplier = "0";
            else:
                $rw = $q->getRowArray();
                $fld_supplier = $rw['supplier_id'];
            endif;
        }else{
            $str="
            SELECT `recid` FROM `mst_vendor` WHERE `recid` = '7805'
        ...
        if ($sd_type == 'CW') {
            $str="
                SELECT trxno FROM trx_sd_attachments WHERE trxno = '$trxno'
            ";
            $q = $this->mylibzdb->myoa_sql_exec($str,'URI: ' . $_SERVER['PHP_SELF'] . chr(13) . chr(10) . 'File: ' . __FILE__  . chr(13) . chr(10) . 'Line Number: ' . __LINE__);
            if($q->getNumRows() == 0) { 
            ...
            $q = $this->mylibzdb->myoa_sql_exec($str,'URI: ' . $_SERVER['PHP_SELF'] . chr(13) . chr(10) . 'File: ' . __FILE__  . chr(13) . chr(10) . 'Line Number: ' . __LINE__);
            if($q->getNumRows() == 0) { 
                echo "<div class=\"alert alert-danger mb-0\" role=\"alert\">[CRPL] Attachment of receipt not found!!!</div>	";
                die();
            } 
        }
        ...
        if ($sd_type == 'CW') {
            ...
            $str="
            SELECT 
                SUM(bbb.`price` * bbb.`qty`) tcost,
                SUM(bbb.`uprice` * bbb.`qty`) tprice,
                SUM(bbb.`qty`) qty,
                aaa.`encd`
            FROM 
            ...
            ";
                ...
                SELECT 
                    SUM(aaa.`ucost` *  (aaa.`qty` * aaa.`convf`)) tcost,
                    SUM(aaa.`uprice` *  (aaa.`qty` * aaa.`convf`)) tprice,
            ...
            $str = "
            SELECT 
                SUM(a.`ucost` * a.`qty`) tcost,
                SUM(a.`uprice` * a.`qty`) tprice,
                SUM(a.`qty`) qty,
                a.`dr_date`
            FROM
                `tap_shipdoc_item` a
            ...
            $str="
                ...
                JOIN 
                    `wshe_crpl_item` bbb 
                ON 
                    (aaa.`recid` = bbb.`crpl_id`)
                LEFT JOIN 
                    `mst_article` ddd 
                ON 
                    (bbb.`mat_rid` = ddd.`recid`)
                WHERE 
                    aaa.`header`= '{$trxno}' AND !(aaa.`frm_plnt_id` ='5')
            ";
            $q = $this->mylibzdb->myoa_sql_exec2($str,'URI: ' . $_SERVER['PHP_SELF'] . chr(13) . chr(10) . 'File: ' . __FILE__  . chr(13) . chr(10) . 'Line Number: ' . __LINE__);
            $rw = $q->getRowArray();
            $fld_subtqty = $rw['qty'];
            ...
            $str_dt_temp = "
                ...
                FROM `wshe_crpl_dt` aaa 
                JOIN `wshe_crpl_item` bbb 
                ON (aaa.`recid` = bbb.`crpl_id`)
                LEFT JOIN `mst_article` ddd 
                ON (bbb.`mat_rid` = ddd.`recid`)
                WHERE aaa.`header`= '{$trxno}' AND !(aaa.`frm_plnt_id` ='5')
                GROUP BY ddd.`ART_CODE`
                ORDER BY ddd.`ART_CODE`
            ";
        }
        ...
        if ($sd_type == 'CW') {
            ...
            ";
            $this->mylibzdb->myoa_sql_exec2($str,'URI: ' . $_SERVER['PHP_SELF'] . chr(13) . chr(10) . 'File: ' . __FILE__  . chr(13) . chr(10) . 'Line Number: ' . __LINE__);
            ...
            $str="
                UPDATE trx_crpl SET `is_receive` = 'Y',`received_by` = '$cuser',`received_date` = NOW() WHERE `crpl_code` = '$trxno'
            ";
            $this->mylibzdb->myoa_sql_exec2($str,'URI: ' . $_SERVER['PHP_SELF'] . chr(13) . chr(10) . 'File: ' . __FILE__  . chr(13) . chr(10) . 'Line Number: ' . __LINE__);
    ...
    public function dashb_inv_intransit_cwo_upld(){
        ...
        $str="
            INSERT INTO `trx_sd_attachments` (
                `trxno`,
                `filename`,
                `upld_date`,
                `upld_by`
    ...
    public function dashb_inv_intransit_old_upld(){
        ...
        function __tap_refresh_data() { 
            try { 
                ...
                $('#mbtn_old_upld').prop('disabled',true);
	...
	public function file_type_check_PDF_only($file_types,$file_count){
		...
		{ 
			...
			if(!in_array($file_type, $allowed_mime_type_arr)){
				echo "<div class=\"alert alert-danger mb-0\" role=\"alert\"><strong>Info.<br/></strong><strong>Error</strong> Please select only <strong>PDF OR PNG</strong> file.</div>";
				die();
			}
    ...
    public function dashb_inv_created_cancel(){
        ...
        if(!$this->myusermod->ua_mod_access_verify($this->db_erp,$this->cuser,'02','0004','000520')) { 
			echo "<div class=\"alert alert-danger mb-0 fw-bold\" role=\"alert\"> Access RESTRICTED!!!</div>";
			die();
		}             
        ...
        if ($sd_type == 'CW') {
            ...
            $str="
            UPDATE trx_crpl SET `is_cancelled` = 'Y',`cancelled_by` = '$cuser',`cancelled_date` = NOW() WHERE `crpl_code` = '$trxno'
            ";
            $this->mylibzdb->myoa_sql_exec2($str,'URI: ' . $_SERVER['PHP_SELF'] . chr(13) . chr(10) . 'File: ' . __FILE__  . chr(13) . chr(10) . 'Line Number: ' . __LINE__);
    ...
    public function dashb_inv_recs_overall_dl(){
        ...
        if((!empty($fld_d2dtfrm) && !empty($fld_d2dtto)) ){
            //$msearchrec = $this->dbx->escape_str($msearchrec);
            $str_date = " AND (date(a.`encd_date`) >= date('{$fld_d2dtfrm}') AND  date(a.`encd_date`) <= date('{$fld_d2dtto}'))";
        }
        ...
        if(!empty($fld_d2brnch)):
            ...
            $rw = $qry->getRowArray();
            $branch_id = $rw['recid'];
            $optn_branch = "AND (a.`branch_id`) = '$branch_id' ";
        endif;


        //AREA AND GROUP
        if(!empty($fld_itmgrparea_s)){
            $str_branchgrparea = " AND (bb.`BRNCH_GROUP` ='$fld_itmgrparea_s')";
            //END BRANCH
        }
        ...
        $chtml = "
            ...
            <tr class=\"header-tr\">
            <th class=\"noborder\" colspan=\"22\">DELIVERY CLAIMS STATUS</th>
            </tr>
            <tr class=\"header-tr\">
            <th class=\"noborder\" colspan=\"22\"></th>
            </tr>
            ...
            <th class=\"noborder\">Dr No.</th>
            <th class=\"noborder\">Dr Date.</th>
            <th class=\"noborder\">Receive Date</th>
            <th class=\"noborder\">Date In</th>
            <th class=\"noborder\">Claim Date</th>
            <th class=\"noborder\">Validation</th>
            <th class=\"noborder\">Validation Date</th>
            <th class=\"noborder\">Verification</th>
            <th class=\"noborder\">Verification Date</th>
            <th class=\"noborder\">Review</th>
            <th class=\"noborder\">Review Date</th>
            <th class=\"noborder\">Correction</th>
            <th class=\"noborder\">Correction Date</th>
            <th class=\"noborder\">Counter</th>
            <th class=\"noborder\">Counter Date</th>	
            ...
            $strqry = "
            SELECT
                CONCAT('''', a.`trx_no`) AS trx_no,
                b.`BRNCH_NAME`,
                c.`VEND_NAME`,
                CONCAT('''', a.`drno`) AS drno,
                IF(a.`dr_date` = '0000-00-00','',a.`dr_date`) dr_date,
                IF(a.`rcv_date` = '0000-00-00','',a.`rcv_date`) rcv_date,
                IF(a.`date_in` = '0000-00-00','',a.`date_in`) date_in,
                a.`claim_date`,
                a.`is_validated`,
                a.`validated_date`,
                a.`is_verified`,
                ...
                a.`is_reviewed`,
                a.`reviewed_date`,
                IF(a.`df_tag` = 'F','Y','N') AS corrected,
                a.`final_date`,
                a.`counter_tag`,
                IF(a.`counter_date` = '0000-00-00 00:00:00','',a.`counter_date`) counter_date
            FROM
                `trx_manrecs_hd` a
            JOIN
                `mst_companyBranch` b
            ...
            JOIN
                mst_vendor c
            ON
                a.`supplier_id` = c.`recid`
            WHERE
                a.`flag` = 'R'
            {$str_date} {$optn_branch} {$str_branchgrparea}
            ...
            if($q->getNumRows() > 0) { 
                ...
                $mpathdest = $mpathdn . 'public/downloads/me'; 
                $cdate     = date('Ymd');
                $cfiletmp  = 'rcvclaims_overall_status_' . $cdate . $this->mylibzsys->random_string(15) .  '.xls' ;
                $cfiledest = $mpathdest . '/' . $cfiletmp;
                $cfilelnk  = site_url() . 'downloads/me/' . $cfiletmp;
                ...
                foreach($qrw as $row):
                    $chtml = "<tr class=\"data-nm\">
                                    ...
                                    <td>".$row['validated_date']."</td>
                                    <td>".$row['is_reviewed']."</td>
                                    <td>".$row['reviewed_date']."</td>
                                    <td>".$row['corrected']."</td>
                                    <td>".$row['final_date']."</td>
                                    <td>".$row['counter_tag']."</td>
                                    <td>".$row['counter_date']."</td>
                                </tr>
                                ";
                    file_put_contents ( $cfiledest , $chtml , FILE_APPEND | LOCK_EX ); 
                    $res++;
